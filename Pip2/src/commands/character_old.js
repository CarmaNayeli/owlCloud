...
async function getActiveCharacter(discordUserId) {
  ...
  let response = await fetchWithTimeout(
    `${process.env.SUPABASE_URL}/rest/v1/rollcloud_characters?discord_user_id=eq.${discordUserId}&is_active=eq.true&select=*&limit=1`,
    {
      headers: {
        'apikey': process.env.SUPABASE_SERVICE_KEY,
        'Authorization': `Bearer ${process.env.SUPABASE_SERVICE_KEY}`
  ...
  if (data.length === 0) {
    response = await fetchWithTimeout(
      `${process.env.SUPABASE_URL}/rest/v1/rollcloud_characters?discord_user_id=eq.${discordUserId}&select=*&order=updated_at.desc&limit=1`,
      {
        headers: {
          'apikey': process.env.SUPABASE_SERVICE_KEY,
          'Authorization': `Bearer ${process.env.SUPABASE_SERVICE_KEY}`
  ...
  if (data.length === 0) {
    ...
    const pairingResponse = await fetchWithTimeout(
      `${process.env.SUPABASE_URL}/rest/v1/rollcloud_pairings?discord_user_id=eq.${discordUserId}&status=eq.connected&select=dicecloud_user_id`,
      {
        headers: {
          'apikey': process.env.SUPABASE_SERVICE_KEY,
          'Authorization': `Bearer ${process.env.SUPABASE_SERVICE_KEY}`
    ...
    if (pairingResponse.ok) {
      ...
      if (pairings.length > 0 && pairings[0].dicecloud_user_id) {
        ...
        let fallbackResponse = await fetchWithTimeout(
          `${process.env.SUPABASE_URL}/rest/v1/rollcloud_characters?user_id_dicecloud=eq.${dicecloudUserId}&is_active=eq.true&select=*&limit=1`,
          {
            headers: {
