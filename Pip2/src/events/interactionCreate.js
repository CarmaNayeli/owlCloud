    ...
    if (!isEnabled) {
      ...
      try {
        await interaction.reply({
          content: `The \`/${interaction.commandName}\` command is disabled on this server. A server admin can enable it in the [Pip Dashboard](https://owlcloud.app/configure-pip).`,
          flags: 64 // ephemeral
        });
      } catch (replyError) {
