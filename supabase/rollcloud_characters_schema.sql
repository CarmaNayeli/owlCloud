-- RollCloud Characters Table Schema
-- Stores character data for cloud sync and Discord bot integration

-- Create the rollcloud_characters table
CREATE TABLE IF NOT EXISTS public.rollcloud_characters (
...
CREATE INDEX IF NOT EXISTS idx_rollcloud_characters_dicecloud_character_id ON public.rollcloud_characters(dicecloud_character_id);
...
COMMENT ON TABLE public.rollcloud_characters IS 'Stores character data for cloud sync and Discord bot integration';
COMMENT ON COLUMN public.rollcloud_characters.id IS 'Primary key for character record';
COMMENT ON COLUMN public.rollcloud_characters.user_id_dicecloud IS 'DiceCloud user ID (Meteor ID) for character ownership';
COMMENT ON COLUMN public.rollcloud_characters.dicecloud_character_id IS 'DiceCloud character ID for reference';
COMMENT ON COLUMN public.rollcloud_characters.character_name IS 'Character display name';
COMMENT ON COLUMN public.rollcloud_characters.race IS 'Character race';
COMMENT ON COLUMN public.rollcloud_characters.class IS 'Character class';
...
COMMENT ON COLUMN public.rollcloud_characters.proficiency_bonus IS 'Proficiency bonus';
COMMENT ON COLUMN public.rollcloud_characters.attributes IS 'Ability scores (STR, DEX, CON, INT, WIS, CHA)';
COMMENT ON COLUMN public.rollcloud_characters.attribute_mods IS 'Ability modifiers';
COMMENT ON COLUMN public.rollcloud_characters.saves IS 'Saving throws';
COMMENT ON COLUMN public.rollcloud_characters.skills IS 'Skill proficiencies';
