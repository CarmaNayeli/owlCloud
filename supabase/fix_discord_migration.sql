-- Fix Discord fields migration - handles existing view properly
-- Run this in your Supabase SQL Editor

-- Step 1: Drop the existing debug view first
DROP VIEW IF EXISTS public.auth_tokens_debug;
RAISE NOTICE 'Dropped existing debug view';

-- Step 2: Add discord_user_id column
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Step 3: Add discord_username column
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- Step 4: Add discord_global_name column
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
